...
def main():
    ...
    pitch_detector = aubio.pitch(
        method="yin", buf_size=buffer_size, hop_size=buffer_size, samplerate=sample_rate
    )
    ...
    def audio_callback(indata, frames, time, status):
        ...
        if pitch > 0 and confidence > 0.3:
            ...
            if stable_note:
                ...
                print(
                    f"\r● {stable_note.name:4} | {stable_note.frequency:6.1f} Hz | {stable_note.confidence:4.2f}    \r",
                    end="",
                    flush=True,
                )
            elif current_note:
                print(
                    f"\r○ {current_note.name:4} | {current_note.frequency:6.1f} Hz | {current_note.confidence:4.2f}    \r",
                    end="",
                    flush=True,
                )
    ...
    for i, device in enumerate(devices):
        if device["max_input_channels"] > 0:
            print(
                f"{i}: {device['name']} (inputs: {device['max_input_channels']}, rate: {device['default_samplerate']})"
            )
    ...
    print(
        f"Buffer size: {buffer_size} samples (~{buffer_size / sample_rate * 1000:.1f}ms)"
    )
