...
class NoteDetector(INoteDetector):
    ...
    def __init__(
        ...
        self._pitch_detector = aubio.pitch(
            "yin", self._hop_size * 2, self._hop_size, self._sample_rate
        )
        ...
        logger.info(
            f"Note detector initialized: sample_rate={sample_rate}, win_size={win_size}, hop_size={hop_size}"
        )
        ...
        try:
            ...
            if len(audio_data) != self._hop_size:
                ...
                if len(audio_data) > self._hop_size:
                    ...
                    padding = np.zeros(
                        self._hop_size - len(audio_data), dtype=audio_data.dtype
                    )
            ...
            if signal_max > 0.01:  # Basic noise gate - same as baseline
                ...
                if self._harmonic_correction:
                    ...
                    if 81.0 < pitch < 84.0:  # Potential E2 that might be E1
                        ...
                        if abs(half_pitch - 41.2) < 2.0:  # Close to E1
                            logger.debug(
                                f"Harmonic correction for E: {pitch:.1f}Hz -> {half_pitch:.1f}Hz (E1)"
                            )
                        ...
                        if abs(half_pitch - 43.65) < 2.0:  # Close to F1
                            logger.debug(
                                f"Harmonic correction for F: {pitch:.1f}Hz -> {half_pitch:.1f}Hz (F1)"
                            )
                            corrected_pitch = half_pitch
                        ...
                        if abs(half_pitch - 46.25) < 2.0:  # Close to F#1
                            logger.debug(
                                f"Harmonic correction for F#: {pitch:.1f}Hz -> {half_pitch:.1f}Hz (F#1)"
                            )
                            corrected_pitch = half_pitch
                ...
                if len(self._note_history) >= self._min_stable_count:
                    ...
                    if note_counts:
                        ...
                        most_common_note = max(note_counts.items(), key=lambda x: x[1])[
                            0
                        ]
                        majority = note_counts[most_common_note] / len(
                            self._note_history
                        )
                        ...
                        if majority >= self._stability_majority:
                            ...
                            if most_common_note != self._current_note:
                                ...
                                logger.info(
                                    f"[{timestamp:.2f}s] {most_common_note} ({pitch:.1f}Hz, conf: {confidence:.2f}, signal: {rms:.4f})"
                                )
                ...
                logger.debug(
                    f"Signal too weak: {signal_max:.4f} (RMS: {rms:.4f}, dB: {db:.1f})"
                )
    ...
    def set_sample_rate(self, sample_rate: int) -> None:
        ...
        if sample_rate != self._sample_rate:
            logger.info(
                f"Updating note detector sample rate from {self._sample_rate} to {sample_rate} Hz"
            )
            self._sample_rate = sample_rate
