...
def run_test(args):
    ...
    try:
        ...
        audio_input = SoundDeviceInput(
            device_id=args.device, sample_rate=args.sample_rate
        )
        ...
        note_detection_service = NoteDetectionService(
            audio_input=audio_input, note_detector=note_detector
        )
    ...
    if detected_notes:
        ...
        for note_name, count in sorted(
            note_counts.items(), key=lambda x: x[1], reverse=True
        ):
