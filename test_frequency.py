...
def main(debug):
    ...
    for i, device in enumerate(devices):
        if device["max_input_channels"] > 0:
            print(
                f"{i}: {device['name']} (inputs: {device['max_input_channels']}, rate: {device['default_samplerate']}Hz)"
            )
    ...
    pitch_method = pitch_methods[
        0
    ]  # Start with yinfft which is often best for guitar/bass

    print(f"\nInitializing pitch detection with method: {pitch_method}")
    print(f"Buffer size: {buffer_size}, Sample rate: {sample_rate}")
    print("Expected low E (bass): 41.2 Hz, low E (guitar): 82.4 Hz")
    ...
    def get_stable_note() -> Optional[DetectedNote]:
        ...
        if stable_note:
            ...
            # If it's the same note, just update the confidence
                ...
                return DetectedNote(
                    stable_note.name, stable_note.frequency, avg_conf, True
                )
        else:
        ...
        if signal_max > 0.01:  # Basic noise gate
            try:
                ...
                if len(bass_magnitude) > 0:
                    ...
                    bass_notes = {"E1": 41.2, "A1": 55.0, "D2": 73.4, "G2": 98.0}

                    # Find the closest bass note
                    closest_note = min(
                        bass_notes.items(), key=lambda x: abs(x[1] - dom_freq)
                    )
                ...
                if debug:
                    ...
                    if dom_freq > 0:
                        ...
                        print(
                            f"\rFrequency: {dom_freq:.1f} Hz | Note: {dom_note} | Signal: {signal_max:.3f}                 \r",
                            end="",
                            flush=True,
                        )
                ...
                if (
                    ...
                    if new_stable_note:
                        ...
                        if debug:
                            print(
                                f"● STABLE: {stable_note.name:4} | {stable_note.frequency:6.1f} Hz | {stable_note.confidence:4.2f}"
                            )
                        else:
                            print(
                                f"\r● STABLE: {stable_note.name:4} | {stable_note.frequency:6.1f} Hz                       \r",
                                end="",
                                flush=True,
                            )
                    elif stable_note:
                        ...
                        if debug:
                            print(
                                f"○ RECENT: {stable_note.name:4} | {stable_note.frequency:6.1f} Hz | {stable_note.confidence:4.2f}"
                            )
                        else:
                            print(
                                f"\r○ RECENT: {stable_note.name:4} | {stable_note.frequency:6.1f} Hz                       \r",
                                end="",
                                flush=True,
                            )
                    else:
                        ...
                        if debug:
                            print(
                                f"  CURRENT: {current_note.name:4} | {current_note.frequency:6.1f} Hz | {current_note.confidence:4.2f}"
                            )
                        else:
                            print(
                                f"\r  CURRENT: {current_note.name:4} | {current_note.frequency:6.1f} Hz                       \r",
                                end="",
                                flush=True,
                            )
