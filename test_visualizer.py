...
BASS_FREQUENCIES = {"E1": 41.20, "A1": 55.00, "D2": 73.42, "G2": 98.00}
...
class BassVisualizer:
    def __init__(self):
        ...
        style = ttk.Style()
        style.configure(
            "Green.Horizontal.TProgressbar", background="green", troughcolor="black"
        )
        ...
        self.device_frame = ttk.LabelFrame(
            self.main_frame, text="Audio Input", padding="5"
        )
        ...
        self.device_menu = ttk.Combobox(
            self.device_frame, textvariable=self.device_var, state="readonly"
        )
        self.device_menu.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=5)

        self.refresh_btn = ttk.Button(
            self.device_frame, text="Refresh", command=self.refresh_devices
        )
        self.refresh_btn.grid(row=0, column=1, padx=5)

        # Frequency display
        self.freq_label = ttk.Label(
            self.main_frame, text="Frequency: --", font=("TkDefaultFont", 24)
        )
        self.freq_label.grid(row=1, column=0, columnspan=2, pady=20)

        # Note display
        self.note_label = ttk.Label(
            self.main_frame, text="Note: --", font=("TkDefaultFont", 24)
        )
        self.note_label.grid(row=2, column=0, columnspan=2, pady=20)

        # Confidence bar
        self.conf_frame = ttk.LabelFrame(
            self.main_frame, text="Confidence", padding="5"
        )
        ...
        self.ref_frame = ttk.LabelFrame(
            self.main_frame, text="Reference Notes", padding="5"
        )
    ...
    def get_note_name(self, frequency):
        ...
        closest_note = min(
            BASS_FREQUENCIES.items(), key=lambda x: abs(x[1] - frequency)
        )
    ...
    def run(self):
        ...
        self.control_btn = ttk.Button(
            self.device_frame, text="Start", command=self.start_detection
        )
